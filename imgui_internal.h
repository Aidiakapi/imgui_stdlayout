...
struct ImGuiViewportP
{
    ...
    void    ClearRequestFlags()         { Base.PlatformRequestClose = Base.PlatformRequestMove = Base.PlatformRequestResize = false; }

    // Calculate work rect pos/size given a set of offset (we have 1 pair of offset for rect locked from last frame data, and 1 pair for currently building rect)
    ImVec2  CalcWorkRectPos(const ImVec2& inset_min) const                           { return ImVec2(Base.Pos.x + inset_min.x, Base.Pos.y + inset_min.y); }
    ImVec2  CalcWorkRectSize(const ImVec2& inset_min, const ImVec2& inset_max) const { return ImVec2(ImMax(0.0f, Base.Size.x - inset_min.x - inset_max.x), ImMax(0.0f, Base.Size.y - inset_min.y - inset_max.y)); }
    void    UpdateWorkRect()            { Base.WorkPos = CalcWorkRectPos(WorkInsetMin); Base.WorkSize = CalcWorkRectSize(WorkInsetMin, WorkInsetMax); } // Update public fields

    // Helpers to retrieve ImRect (we don't need to store BuildWorkRect as every access tend to change it, hence the code asymmetry)
    ImRect  GetMainRect() const         { return ImRect(Base.Pos.x, Base.Pos.y, Base.Pos.x + Base.Size.x, Base.Pos.y + Base.Size.y); }
    ImRect  GetWorkRect() const         { return ImRect(Base.WorkPos.x, Base.WorkPos.y, Base.WorkPos.x + Base.WorkSize.x, Base.WorkPos.y + Base.WorkSize.y); }
...
namespace ImGui
{
    ...
    inline ImDrawList*      GetForegroundDrawList(ImGuiWindow* window) { return GetForegroundDrawList((ImGuiViewport*) window->Viewport); }
